...
def get_julia_url() -> tuple[str, str]:
    desired_version = os.environ["JULIA_VERSION"]
    
    LOGGER.info("Downloading Julia versions information")
    versions = requests.get(
        "https://julialang-s3.julialang.org/bin/versions.json"
    ).json()
    stable_versions = {k: v for k, v in versions.items() if k == desired_version }

    latest_version_files = stable_versions[desired_version]["files"]
...
if __name__ == "__main__":
    ...
    julia_url, julia_version = get_julia_url()
